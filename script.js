...
function setMobileMenuState(isOpen) {
  /* 햄버거 버튼 안에 있는 <img> 태그를 찾아 변수에 저장 (아이콘 변경을 위해) */
  const hamburgerIcon = hamburgerBtn.querySelector('img');

  if (isOpen) {
    /* --- 메뉴 열기 --- */
    mobileMenu.classList.remove('hidden');                 // hidden 제거 → 메뉴 카드 보임
    hamburgerIcon.src = 'images/icon-close.svg';           // 비유: 영업 시작 안내판을 'X(닫기)'로 바꾸는 것처럼 아이콘 변경
    hamburgerIcon.width = 24;                              // 새로 바뀐 닫기 아이콘 크기를 24x24px로 세팅
    ...
    hamburgerIcon.src = 'images/icon-hamburger.svg';       // 아이콘을 다시 원래의 '햄버거(≡)' 모양으로 돌려놓음
    hamburgerIcon.width = 32;                              // 원래 햄버거 아이콘 비율(32x18px)로 원상복구
    hamburgerIcon.height = 18;                             
